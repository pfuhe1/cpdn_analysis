...
# Regression
	regr=f_gy.variables['regr'][0,:]
#	var=np.ma.masked_values(ratio,1.e20)
	regr=np.ma.masked_values(regr,3.e33)
...
#	var=np.ma.masked_values(ratio,1.e20)
#	trend_ak=np.ma.masked_values(trend_ak,-999.)
...
###############################
	...
	plt.title('a)')
	c=m.pcolor(x,y,regr*0.85,vmin=-3.,vmax=3.)
	m.drawcoastlines()
	m.drawcountries()
	...
	axes_list.append(plt.gca())
	plt.title('b)')
	c=m.pcolor(x2,y2,trend_ak,vmin=-3.,vmax=3)#,np.arange(0.6,1.05,.05),extend='both')
	m.drawcoastlines()
	m.drawcountries()
	m.drawparallels(circles)
	m.drawmeridians(meridians)
	...
	axes_list.append(plt.gca())
	plt.title('c)')
	c=m.pcolor(x3,y3,trend2014,vmin=-3.,vmax=3)
	m.drawcoastlines()
	m.drawcountries()
	m.drawparallels(circles)
	m.drawmeridians(meridians)
	...
	axes_list.append(plt.gca())
	plt.title('d)')
	c=m.pcolor(x3,y3,trendclim,vmin=-3.,vmax=3)
	m.drawcoastlines()
	m.drawcountries()
	m.drawparallels(circles)
	m.drawmeridians(meridians)
	...
	plt.savefig(folder+'trend_combined.png')
