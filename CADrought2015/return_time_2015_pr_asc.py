...
def return_time_swe(hist_data,clim_data,nat_data):
...
#	allf_mec="MediumBlue"
	...
	ax.set_ylabel("Total Precipitation less than this amount (mm)",fontsize=16)
...
#	print 'histx',x_data_all.min(),x_data_all.max()
	...
	nsims=nens_hist +nens_nat + nens_clim# Plus number from other simulations
...
#	ax.set_yticklabels(labels)
	...
	plt.legend(loc='best')
	fname_out="return_time_pr_pnw_test.png"
...
def main():
	...
	# All Forcings
	fnames_pr_hist=glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch1/ga.pd/field90/field90_????_2014-11.nc')
	...
	for i,fname in enumerate(fnames_pr_hist):
		...
		if len(monthfiles)==6:		
			for fname2 in monthfiles:
				tmp=netcdf_file(fname,'r').variables['ga.pd'][:,0,:].sum(0) # Sum of precip over time 
				tmpsum=tmpsum+calif_mean(tmp)
			pr_hist[i]=tmpsum*86400.
		else:
			pr_hist[i]=np.nan
		
	# All Forcings with Clim SSTs
	fnames_pr_clim=glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch2/ga.pd/field90/field90_????_2014-11.nc')
	pr_clim=np.zeros([len(fnames_pr_clim)])
	for i,fname in enumerate(fnames_pr_clim):
		tmpsum=0
		monthfiles=glob.glob(fname[:-10]+'*') # TODO, could loop over specific months instead
		if len(monthfiles)==6:
			for fname2 in monthfiles:
	...
	for subbatch in nat_subbatches:
		fnames_pr_nat=fnames_pr_nat+glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch'+str(subbatch)+'/ga.pd/field90/field90_????_2014-11.nc')
	...
	for i,fname in enumerate(fnames_pr_nat):
		...
		if len(monthfiles)==6:		
			for fname2 in monthfiles:
				tmp=netcdf_file(fname,'r').variables['ga.pd'][:,0,:].sum(0) # Sum of precip over time 
				tmpsum=tmpsum+calif_mean(tmp)
			pr_nat[i]=tmpsum*86400.
		else:
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
			pr_nat[i]=np.nan
	
	# Remove out nans
	pr_hist=pr_hist[np.isfinite(pr_hist)]
	pr_clim=pr_clim[np.isfinite(pr_clim)]
	pr_nat=pr_nat[np.isfinite(pr_nat)]
	
	print 'hist',pr_hist.min(),pr_hist.mean(),pr_hist.max()
	print 'clim',pr_clim.min(),pr_clim.mean(),pr_clim.max()
	print 'nat',pr_nat.min(),pr_nat.mean(),pr_nat.max()
	return_time_swe(pr_hist,pr_clim,pr_nat)
