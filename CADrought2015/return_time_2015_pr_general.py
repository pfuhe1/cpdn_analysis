...
def return_time_pr(i,fig,datad,mecs,cols,region,fname_out=False):
	...
	ax.set_title(region+" Nov2014-Apr2015 Precipitation\n"+str(nsims).zfill(5)+" Simulations")
...
# Code to write out pickle files to csv.
	...
	fnames_data_hist=fnames_data_hist+glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch1/ga.pe/field90/field90_????_2014-11.nc')
		...
		if umid not in data_dict_hist.keys():
			...
			try:	
				for monthstr in months:
					...
					tmp=netcdf_file(fname2,'r').variables['field90'][0,0,:]
					for j,region in enumerate(regions):
						tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
				data_dict_hist[umid]=tmpsum*60*60*24*30 #Total preccipitaion
			except Exception,e:
	...
	# Climatology
	fnames_data_clim=glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch29/ga.pe/field90/field90_????_2014-11.nc')
		...
		if umid not in data_dict_clim.keys():
			...
			try:	
				for monthstr in months:
					...
					for j,region in enumerate(regions):
						tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
				data_dict_clim[umid]=tmpsum*60*60*24*30 #Total preccipitaion
			except Exception,e:
	...
	# Natural Forcings
	nat_subbatches=range(17,29)+range(3,15)
	fnames_data_nat=[]
	for subbatch in nat_subbatches:
		fnames_data_nat=fnames_data_nat+glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch'+str(subbatch)+'/ga.pe/field90/field90_????_2014-11.nc')
		...
		if umid not in data_dict_nat.keys():
			...
			try:	
				for monthstr in months:
					...
					for j,region in enumerate(regions):
						tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
				data_dict_nat[umid]=tmpsum*60*60*24*30 #Total preccipitaion in mm for each month
			except Exception,e:
				print "Error loading files",e
				continue
				
	# Natural Forcings V2
	nat_subbatches=range(30,42)
	fnames_data_nat2=[]
	for subbatch in nat_subbatches:
		fnames_data_nat2=fnames_data_nat2+glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch'+str(subbatch)+'/ga.pe/field90/field90_????_2014-11.nc')
	random.shuffle(fnames_data_nat)
	data_nat=np.zeros([len(regions),len(fnames_data_nat2)])
		...
		if umid not in data_dict_nat2.keys():
			...
			try:	
				for monthstr in months:
					...
					for j,region in enumerate(regions):
						tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
				data_dict_nat2[umid]=tmpsum*60*60*24*30 #Total preccipitaion in mm for each month
			except Exception,e:
				print "Error loading files",e
				continue
...
#		imax=int(min([data_hist.shape[0],data_clim.shape[0],data_nat.shape[0]])*1.5)
	print 'Max ensembles included:',imax	
	# Count number of simulations used:
#	nens_hist=min(data_hist.shape[0],imax)
#	nens_nat=min(data_nat.shape[0],imax)
#	nens_clim=min(data_clim.shape[0],imax)
...
#	print 'ensembles:',nens_hist,nens_clim,nens_nat,nens_total
	...
	alpha=0.07
	for a in range(3,imax):
		idx=a+int(math.exp((a)*alpha))
		ilist.append(idx)
		if idx>=imax:
			break
	...
	for i,region in enumerate(regions):
		...
		fout='/home/cenv0437/cpdn_analysis/CADrought2015/archive_fix/pr_'+region+'_n'+str(nens_total)
		webfile='/home/cenv0437/cpdn_analysis/CADrought2015/web_content/pr_'+region
			
		# Plot histograms of data
#		plt.figure()
#		for label,data in datad.iteritems():
#			print label,np.isfinite(data).sum()
#			plt.hist(data[np.isfinite(data)],50,histtype='stepfilled',normed=1,facecolor=cols[label],edgecolor=cols[label],alpha=0.2,label=label)
#			plt.axvline(x=np.nanmean(data),color=cols[label])
#		plt.legend(loc='upper left')
#		plt.savefig('archive_extra/pr_hist_'+region+'.png')
		
		# Make still image
		f_ext='.png'
		return_time_pr(imax,plt.figure(),datad,mecs,cols,region,fname_out=fout+f_ext)
