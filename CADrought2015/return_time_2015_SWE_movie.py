...
def return_time_swe_clim(i,fig,hist_data,clim_data,nat_data,out_dir=False):

	# Number of ensemble members
	nens_hist=hist_data.shape[0]
	nens_nat=nat_data.shape[0]
	nens_clim=clim_data.shape[0]
...
#	allf_mec="MediumBlue"
	...
	# Clim data
	y_data_all, x_data_all = calc_return_times(clim_data,
											   direction="ascending", 
											   period=1)
	...
	conf_all_5=conf_all[0,:].squeeze()
	conf_all_95=conf_all[1,:].squeeze()
	cl1=ax.fill_between(x_data_all,conf_all_5,conf_all_95,facecolor=clim_mec,edgecolor=clim_mec,alpha=0.3,linewidth=1.5,zorder=4)
	...
	ax.set_title("California April 1 2015 Snow Amount\n"+str(nsims).zfill(5)+" Simulations")

	ax.set_ylim(0,100) #0,450
	ax.set_xlim(1,1e3) 
	labels=['1/1','1','1/10','1/100','1/1000']
	...
	if out_dir:
		if not os.path.exists(out_dir):
        		os.mkdir(out_dir)
		fname_out=os.path.join(out_dir,"return_time_swe_pnw_clim"+str(nsims).zfill(5)+".png")
		fig.savefig(fname_out,dpi=28.75*2)


####################################################################

def return_time_swe_log(i,fig,hist_data,nat_data,out_dir=False):
...
#	allf_mec="MediumBlue"
	...
	fig.subplots_adjust(bottom=0.15)
	ax.set_ylabel("Threshold of snow amount (mm)",fontsize=16)
	ax.set_xlabel("Chance of having snow less than threshold on April 1",fontsize=16)
	plt.setp(ax.get_xticklabels(),fontsize=12)
	plt.setp(ax.get_yticklabels(),fontsize=12)
	...
	ax.set_title("California April 1 Snow Amount\n"+str(nsims).zfill(5)+" Simulations")

	ax.set_ylim(0.01,90) #0,450
	ax.set_xlim(1,1e3) 

	xlabels=['1/1','1','1/10','1/100','1/1000']
#	ylabels=['1/1','1','10','100']
...
#	ax.set_yticklabels(ylabels)
	...
	if out_dir:
		if not os.path.exists(out_dir):
        		os.mkdir(out_dir)
		fname_out=os.path.join(out_dir,"return_time_swe_log_"+str(nsims).zfill(5)+".png")
		fig.savefig(fname_out,dpi=28.75*2)
		
####################################################################

def return_time_swe_semilog(i,fig,hist_data,nat_data,out_dir=False):
...
#	allf_mec="MediumBlue"
	...
	if out_dir:
		if not os.path.exists(out_dir):
        		os.mkdir(out_dir)
		fname_out=os.path.join(out_dir,"return_time_swe_semilog_"+str(nsims).zfill(5)+".png")
		fig.savefig(fname_out,dpi=28.75*2)
...
def main():
	...
	# Actual
	fnames_swe_allforc=glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch1/ga.pd/field93/field93_????_2015-04.nc')
	random.shuffle(fnames_swe_allforc)
	swe_allforc=np.zeros([len(fnames_swe_allforc)])
	for i,fname in enumerate(fnames_swe_allforc):
		print os.path.basename(fname)
		tmp=netcdf_file(fname,'r').variables['field93'][0,0,:] # Take first of April. 
		swe_allforc[i]=calif_mean(tmp)
		
	
#	# Actual with Clim SSTs
#	fnames_swe_clim=glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch2/ga.pd/field93/field93_????_2015-04.nc')
#	swe_clim=np.zeros([len(fnames_swe_clim)])
#	for i,fname in enumerate(fnames_swe_clim):
#		print os.path.basename(fname)
#		tmp=netcdf_file(fname,'r').variables['ga.pd'][0,0,:] # Take first of April. 
#		swe_clim[i]=calif_mean(tmp)
	
	# Natural Forcings
	nat_subbatches=range(3,15)
	fnames_swe_nat=[]
	for subbatch in nat_subbatches:
		fnames_swe_nat=fnames_swe_nat+glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch'+str(subbatch)+'/ga.pd/field93/field93_????_2015-04.nc')
	random.shuffle(fnames_swe_nat)
	swe_nat=np.zeros([len(fnames_swe_nat)])
	for i,fname in enumerate(fnames_swe_nat):
		print os.path.basename(fname)
		tmp=netcdf_file(fname,'r').variables['field93'][0,0,:] # Take first of April. 
		swe_nat[i]=calif_mean(tmp)
	...
	if make_video:
		...
		#Maximum number of ensembles members to include
		imax=100
#		imax=max(swe_allforc.shape[0],swe_nat.shape[0])
#		imax=min(swe_allforc.shape[0],swe_nat.shape[0])
		...
		fig=plt.figure()			
		anim = animat.FuncAnimation(fig, return_time_swe_semilog,
					   fargs=(fig,swe_allforc,swe_nat),
					   frames=ilist, interval=20, blit=True)
	 
	 	fout='/home/cenv0437/cpdn_analysis/CADrought2015/web_content/SWE_movie_california.mp4'
		print 'created animation...',
		anim.save(fout,writer='ffmpeg',fps=20,extra_args=['-vcodec', 'libx264'])
		print "saved as",fout
	
	else:
		return_time_swe_semilog(imax,plt.figure(),swe_allforc,swe_nat,'.')
