...
def return_time_swe(i,fig,data_hist,data_clim,data_nat,region,out_dir=False):
	...
	l1=ax.semilogx(x_data_all,y_data_all, marker='o',markersize=7,
				   ...
				   label="Natural 2014-15",zorder=5)			   
	# Confidence interval
	...
	l1=ax.semilogx(x_data_all,y_data_all, marker='o',markersize=7,
				   ...
				   label="Climatology 2000-10",zorder=5)
	# Confidence Interval	   
	...
	ylims={'California':(0,100),'Oregon':(0,100),'Washington':(0,50)}
	...
	if out_dir:
		if not os.path.exists(out_dir):
			os.mkdir(out_dir)
		fname_out=os.path.join(out_dir,"return_time_swe_"+str(nsims).zfill(5)++"_"+region+".png")
	...
	for i,fname in enumerate(fnames_data_hist):
		try:	
			...
			for j,region in enumerate(regions):
				data_hist[j,i]=region_mean(tmp,region)
		except:
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
			data_hist[j,i]=np.nan
			raise
	...
	for i,fname in enumerate(fnames_data_clim):
		try:	
			...
			tmp=netcdf_file(fname,'r').variables['field93'][0,0,:] # Take first of april only
			for j,region in enumerate(regions):
				data_clim[j,i]=region_mean(tmp,region)
		except:
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
			data_clim[j,i]=np.nan
			raise
	...
	for i,fname in enumerate(fnames_data_nat):
		try:	
			...
			tmp=netcdf_file(fname,'r').variables['field93'][0,0,:] # Take first of april only
			for j,region in enumerate(regions):
				data_nat[j,i]=region_mean(tmp,region)
		except:
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
			data_nat[j,i]=np.nan
			raise
...
#		imax=int(min([data_hist.shape[1],data_clim.shape[1],data_nat.shape[1]])*1.5) # Use this for live results
		...
		for i,region in enumerate(regions):		
			...
			fout='/home/cenv0437/cpdn_analysis/CADrought2015/archive_all/swe_'+region+'_n'+str(nens_total)+'.mp4'
			
			print 'created animation...',
			anim.save(fout,writer='ffmpeg',fps=20,extra_args=['-vcodec', 'libx264'])
			print "saved as",fout
			
			webfile='/home/cenv0437/cpdn_analysis/CADrought2015/web_content/swe_'+region+'.mp4'
#			if os.path.exists(webfile):
#				os.remove(webfile)
#			os.symlink(fout,webfile)
	else:
		return_time_swe(imax,plt.figure(),data_hist[i,:],data_clim[i,:],data_nat[i,:],region,outdir='.')
