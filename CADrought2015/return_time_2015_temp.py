...
def return_time_temp(hist_data,clim_data,nat_data):
...
#	allf_mec="MediumBlue"
	...
	ax.set_ylabel("Temperature threshold",fontsize=16)
	...
	ax.set_title("California Dec2014-Jan2015 Temp - "+str(nsims)+" Simulations")
...
#	ax.set_ylim(0,ymax) #0,450
	...
	plt.legend(loc='best')
	fname_out="return_time_temp_pnw_test.png"
	...
	for i,fname in enumerate(fnames_temp_hist):
		...
		try:	
			for monthstr in months:
				...
				tmp=netcdf_file(fname2,'r').variables['ga.pe'][0,0,:]
...
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
	...
	fnames_temp_clim=glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch2/ga.pe/field16/field16_????_2014-11.nc')
	temp_clim=np.zeros([len(fnames_temp_clim)])
	for i,fname in enumerate(fnames_temp_clim):
		tmpsum=0
		try:	
			for monthstr in months:
				fname2=fname[:-10]+monthstr+'.nc'
...
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
	...
	print 'hist',temp_hist.min(),temp_hist.mean(),temp_hist.max()
	print 'clim',temp_clim.min(),temp_clim.mean(),temp_clim.max()
	print 'nat',temp_nat.min(),temp_nat.mean(),temp_nat.max()
	return_time_temp(temp_hist,temp_clim,temp_nat)
