	...
	for i,fname in enumerate(fnames_data_hist):
		...
		if umid not in data_dict_hist.keys():
			...
			try:	
				...
				data_dict_hist[umid]=(tmpsum/3)-273.15
			except Exception,e:
		...
		if umid not in data_dict_clim.keys():
			...
			try:	
				for monthstr in months:
					...
					for j,region in enumerate(regions):
						tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
				data_dict_clim[umid]=(tmpsum/3)-273.15
			except Exception,e:
				print "Error loading files",e
				continue
...
#	random.shuffle(fnames_data_nat)
		...
		if umid not in data_dict_nat.keys():
			...
			try:	
				for monthstr in months:
					...
					for j,region in enumerate(regions):
						tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
				data_dict_nat[umid]=(tmpsum/3)-273.15
			except Exception,e:
				print "Error loading files",e
				continue
	...
	for subbatch in nat_subbatches2:
		fnames_data_nat2=fnames_data_nat2+glob.glob('/home/cenv0437/scratch/hadam3p_pnw/batch181/sub-batch'+str(subbatch)+'/ga.pe/field16/field16_????_2014-11.nc')
#	random.shuffle(fnames_data_nat)
		...
		if umid not in data_dict_nat2.keys():
			...
			try:	
				for monthstr in months:
					...
					for j,region in enumerate(regions):
						tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
				data_dict_nat2[umid]=(tmpsum/3)-273.15
			except Exception,e:
				print "Error loading files",e
				continue
	...
	for i,region in enumerate(regions):
		...
		fout='/home/cenv0437/cpdn_analysis/CADrought2015/archive_fix/temp_'+region+'_n'+str(nens_total)
		webfile='/home/cenv0437/cpdn_analysis/CADrought2015/web_content/temp_'+region
...
#		plt.savefig('archive_extra/temp_hist_'+region+'.png')

		# Make still image
		f_ext='.png'
		return_time_temp(imax,plt.figure(),datad,mecs,cols,region,fname_out=fout+f_ext)
