...
def return_time_pr(i,fig,data_hist,data_clim,data_nat,region,out_dir=False):
	...
	### Hist data
	# Return Times
	y_data_all, x_data_all = calc_return_times(data_hist,  direction="ascending",  period=1)
	...
	# Confidence Interval	   
	conf_all = calc_return_time_confidences(data_hist,direction="ascending",bsn=1e3)  
	...
	### Climatology data
	# Return Times
	y_data_all, x_data_all = calc_return_times(data_clim,  direction="ascending",  period=1)
	l1=ax.semilogx(x_data_all,y_data_all, marker='o',markersize=7,
				   linestyle='None',mec=clim_mec,mfc=season_col_clim,
				   color=season_col_clim,fillstyle='full',
				   label="Climatology",zorder=5)
	# Confidence Interval	   
	conf_all = calc_return_time_confidences(data_clim,direction="ascending",bsn=1e3)  
	conf_all_5=conf_all[0,:].squeeze()
	conf_all_95=conf_all[1,:].squeeze()
	...
	### Nat data
	# Return Times
	y_data_all, x_data_all = calc_return_times(data_nat,direction="ascending", period=1)
	l1=ax.semilogx(x_data_all,y_data_all, marker='o',markersize=7,
				   linestyle='None',mec=nat_mec,mfc=season_col_nat,
				   color=season_col_nat,fillstyle='full',
				   label="Natural",zorder=5)			   
	# Confidence interval
	conf_all = calc_return_time_confidences(data_nat,direction="ascending",bsn=1e3)  
	conf_all_5=conf_all[0,:].squeeze()
	conf_all_95=conf_all[1,:].squeeze()
...
def main():
	...
	for i,fname in enumerate(fnames_data_hist):
		...
		try:	
			...
			data_hist[:,i]=tmpsum*60*60*24*30 #Total preccipitaion
		except:
	...
	for i,fname in enumerate(fnames_data_clim):
		...
		try:	
			for monthstr in months:
				...
				for j,region in enumerate(regions):
					tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
			data_clim[:,i]=tmpsum*60*60*24*30 #Total preccipitaion
		except:
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
	...
	for i,fname in enumerate(fnames_data_nat):
		...
		try:	
			for monthstr in months:
				...
				for j,region in enumerate(regions):
					tmpsum[j]=tmpsum[j]+region_mean(tmp,region)
			data_nat[:,i]=tmpsum*60*60*24*30 #Total preccipitaion in mm for each month
		except:
#			raise Exception('wrong number of files: '+str(len(monthfiles)))
...
#		imax=int(min([data_hist.shape[1],data_clim.shape[1],data_nat.shape[1]])*1.5)
		...
		for i,region in enumerate(regions):		
			...
			fout='/home/cenv0437/cpdn_analysis/CADrought2015/archive_all/pr_'+region+'_n'+str(nens_total)+'.mp4'
			
			print 'created animation...',
			anim.save(fout,writer='ffmpeg',fps=20,extra_args=['-vcodec', 'libx264'])
			print "saved as",fout
			
			webfile='/home/cenv0437/cpdn_analysis/CADrought2015/web_content/pr_'+region+'.mp4'
#			if os.path.exists(webfile):
#				os.remove(webfile)
#			os.symlink(fout,webfile)
	else:
		return_time_pr(imax,plt.figure(),data_hist[i,:],data_clim[i,:],data_nat[i,:],region,outdir='.')
