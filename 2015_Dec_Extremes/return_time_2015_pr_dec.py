...
def main():
	...
	f_pickle=pkl_dir+'/pr_results_dec_'+region+'.pkl'
...
#		datad[key]=data-bias
	...
	# Threshold relative to actual ensemble
        ...
        tmp.sort()
        thresholds.append(tmp[-(n_hist/nyears)])

        tmp=datad['2015 Natural']
        tmp.sort()
        thresholds.append(tmp[-(n_nat/nyears)])

	tmp=datad['1985-2013 Climatology']
	tmp.sort()
	thresholds.append(tmp[-(n_clim/nyears)])
	
	# Observed 2015 threshold
#			thresh=5.0
	print region
	for i,key in enumerate(datad.keys()):
		condition=datad['2015 Actual']>thresholds[i]
		count_hist=condition.sum(0)/(n_hist*1.0)
		return_hist=ret_time_bootstrap_stats(datad['2015 Actual'],thresholds[i])
	
		condition=datad['2015 Natural']>thresholds[i]
		count_nat=condition.sum(0)/(n_nat*1.0)
		return_nat=ret_time_bootstrap_stats(datad['2015 Natural'],thresholds[i])
	
		condition=datad['1985-2013 Climatology']>thresholds[i]
		count_clim=condition.sum(0)/(n_clim*1.0)
		return_clim=ret_time_bootstrap_stats(datad['1985-2013 Climatology'],thresholds[i])
	
		ratio_anth=(count_hist)/(count_nat)
		ratio_anom=(count_hist)/count_clim

		print "1 in "+str(nyears)+" "+key+"event"
#			print '2015 event'
