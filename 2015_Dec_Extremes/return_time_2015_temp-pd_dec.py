...
def main():
	...
	f_pickle=pkl_dir+'/temp_results_dec-pd_'+region+'.pkl'
	...
	# Actual 
	fnames_data_hist=glob.glob('/home/cenv0437/scratch/hadam3p_eu/batch266/ga.pd/field16/field16_????_2015-11.nc')
	for i,fname in enumerate(fnames_data_hist):
		...
		try:
			if umid not in data_hist.keys():
				...
				tmp0=netcdf_file(fname2,'r').variables['field16'][:,0,:].mean(0)
				tmp1=netcdf_file(fname2.replace('field16','field16_1'),'r').variables['field16_1'][:,0,:].mean(0)
				data_hist[umid]=region_mean((tmp0+tmp1)/2.,region,lat_coords,lon_coords)-273.15
		except Exception,e:
	...
	# Natural 
	fnames_data_nat=glob.glob('/home/cenv0437/scratch/hadam3p_eu/batch267/ga.pd/field16/field16_????_2015-11.nc')
	for i,fname in enumerate(fnames_data_nat):
		...
		try:
			if umid not in data_nat.keys():
				...
				tmp0=netcdf_file(fname2,'r').variables['field16'][:,0,:].mean(0)
				tmp1=netcdf_file(fname2.replace('field16','field16_1'),'r').variables['field16_1'][:,0,:].mean(0)
				data_nat[umid]=region_mean((tmp0+tmp1)/2.,region,lat_coords,lon_coords)-273.15
		except Exception,e:
			print "Error loading files",e
			continue
			
	# Clim (all months)
	fnames_data_clim=glob.glob('/home/cenv0437/scratch/hadam3p_eu/batch211/ga.pd/field16/field16_????_????-12.nc')
	for fname in fnames_data_clim:
		...
		try:
			if umid not in data_clim.keys():
				...
				tmp0=netcdf_file(fname2,'r').variables['field16'][:,0,:].mean(0)
				tmp1=netcdf_file(fname2.replace('field16','field16_1'),'r').variables['field16_1'][:,0,:].mean(0)
				data_clim[umid]=region_mean((tmp0+tmp1)/2.,region,lat_coords,lon_coords)-273.15
		except Exception,e:
			print "Error loading files",e
			continue
...
#			print '2015 event'
	...
	fout='/home/cenv0437/cpdn_analysis/2015_Dec_Extremes/figs/temp_return_dec-pd_'+region+'_uncorrected'
	# Make still image
	f_ext='.png'
