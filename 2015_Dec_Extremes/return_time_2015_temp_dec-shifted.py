...
# 					print os.path.basename(fname2)
# 					tmp0=region_mean2(netcdf_file(fname2,'r').variables['field16'][:,0,:],region,lat_coords,lon_coords)-273.15
# 					tmp1=region_mean2(netcdf_file(fname2.replace('field16','field16_1'),'r').variables['field16_1'][:,0,:],region,lat_coords,lon_coords)-273.15
# 					tmp[i*30:(i+1)*30]=(tmp0+tmp1)/2.
# 				data_hist[umid]=tmp[9:40].mean(0)
# 		except Exception,e:
...
# 					tmp1=region_mean2(netcdf_file(fname2.replace('field16','field16_1'),'r').variables['field16_1'][:,0,:],region,lat_coords,lon_coords)-273.15
# 					tmp[i*30:(i+1)*30]=(tmp0+tmp1)/2.
# 				data_clim[umid]=tmp[9:40].mean(0)		except Exception,e:
# 			print "Error loading files",e
# 			continue
