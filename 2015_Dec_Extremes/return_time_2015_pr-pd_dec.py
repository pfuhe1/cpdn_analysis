...
def main():
	...
	f_pickle=pkl_dir+'/pr_results_dec-pd_'+region+'.pkl'
	...
	for i,fname in enumerate(fnames_data_hist):
		...
		try:
			if umid not in data_hist.keys():
				...
				tmp0=netcdf_file(fname2,'r').variables['field90'][:,0,:].mean(0)
	...
	for fname in fnames_data_clim:
		...
		try:
			if umid not in data_clim.keys():
				...
				tmp0=netcdf_file(fname,'r').variables['field90'][:,0,:].mean(0)
				data_clim[umid]=region_mean(tmp0,region,lat_coords,lon_coords)*60*60*24
				print data_clim[umid]
		except Exception,e:
			print "Error loading files",e
			continue
...
#			print '2015 event'
	...
	fout='/home/cenv0437/cpdn_analysis/2015_Dec_Extremes/figs/pr_return_dec-pd_'+region+'_uncorrected'
	# Make still image
	f_ext='.png'
