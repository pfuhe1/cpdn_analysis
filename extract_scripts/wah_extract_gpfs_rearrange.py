...
#function to return netcdf file name from zip
def nc_filename(umid,syear,smon,zip_num,pp_code):
	...
	return umid+pp_code+dec_code+year_code+month_str+'.nc'
...
def make_directories2(output_dir, field_list, n_valid,umid):
	# make the output top directory
	c_path = output_dir
	download=False
	if not os.path.exists(c_path):
	...
	for field in field_list:
		try: 
			field_valid=int(field[6].split('_')[0])
		except:
			field_valid=n_valid
		# make the pattern directory (i.e. pattern for global / regional / regional daily etc.)
		f_path = c_path + "/" + field[0]
		...
		if not os.path.exists(f_path):
			os.mkdir(f_path)
		if len(glob.glob(f_path+'/*'+umid+'*')) < field_valid:
...
def extract(taskpath, field_list, output_dir, temp_dir, n_valid,gpfs):
...
if __name__ == "__main__":
	...
	taskdirs= set(map(os.path.dirname,urls))
...
#	temp_dir = tempfile.mkdtemp(dir='/gpfs/projects/cpdn/scratch/cenv0437/')
	...
	for u in list(taskdirs):
		...
		base_path,all_netcdfs=extract(u, field_list, output_dir, temp_dir, n_valid,gpfs)
